...
class MqttTopicParser {
  ...
  public static parse(path: string, allowWildcard = false): { parts: string[]; wildcard: boolean } {
    if (typeof path !== "string" || !path.length) throw new Error("A device path is required");
    const parts = [];
    let part = "";
    let hash = false;
    for (let i = 0; i < path.length; i++) {
      ...
      if (char === "\\") {
        const next = path[++i];
        if (!next || !"\\.#".includes(next)) throw new Error("Invalid path escape");
        part += next;
      } else if (char === ".") {
        if (hash) throw new Error("# is only allowed as the final subscription component");
        parts.push(part);
        part = "";
      } else {
        part += char;
...
if (typeof module !== "undefined" && module.exports) module.exports = MqttTopicParser;
else (globalThis as any).MqttTopicParser = MqttTopicParser;
